...
{#if body && textSide && listSide}
    <section class="body">
        <Container>
            <div class="body-row">
                <div class="text-side">
                    <Headline title={replaceStringWithCity(textSide.title, page.data?.currentCity || '')} level={2} levelStyle={2} />
                    {#if Array.isArray(textSide.paragraphs) && textSide.paragraphs.length}
                        <div class="text-side__content">
                            {#each textSide.paragraphs as text}
                                <Paragraph text={replaceStringWithCity(text, page.data?.currentCity || '')} size="p" weight="400" />
                            {/each}
                        </div>
                    {/if}
                </div>
                {#if Array.isArray(listSide.items) && listSide.items.length}
                    <div class="list-side">
                        <Headline title={replaceStringWithCity(listSide.title, page.data?.currentCity || '')} level={3} levelStyle={4} />
                        <ul class="list">
